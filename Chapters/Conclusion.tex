...
whether an agent should change its policies according to the time it has or
should the
...
The planning component of CloudAssure uses the \gls{mdp} value and policy
iteration algorithm. These however do not deal with uncertainty. If an
organization for some reason does have not a trust value for a particular node/user
then the system may function correctly. Therefore, we recommend that during the
implementation, if
trust value cannot be evaluated for a particular node, assign the nodes with
some informed or estimated trust metrics. This allows the system to continue generating
decisions. 

Secondly, as we have shown, the \gls{mdp} is computationally expensive. In
situations where the number of nodes is large, the \gls{mdp} will take
a considerable about of time to generate a decision. If this case, we recommend
the use of localized subgraph, where only certain nodes/users relevant for 
decision making are considered. By this way we can reduce
